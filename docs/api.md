...
The service token backend exposes its API at `/api/service-tokens`. All endpoints require a valid Backstage user session with the `service-tokens.admin` permission.
...
| `403 Forbidden` | Token is valid but the user lacks `service-tokens.admin` permission |
