...
- The guest user has the `service-tokens.admin` permission (granted via `serviceTokens.admin.userEntityRefs` in `app-config.yaml`).
...
The permission policy grants `service-tokens.admin` only to users listed in `serviceTokens.admin.userEntityRefs`. To demonstrate the 403 path, add a second user to `org.yaml` and `app-config.yaml` who is **not** in the admin list, then obtain their token.
