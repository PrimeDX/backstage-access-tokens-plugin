...
| `permission.js` | Exports `serviceTokensAdminPermission` — the single permission object used by both the backend and the consuming app's permission policy |
...
Admin user (browser)
  → POST /api/service-tokens
    → expressRouter: authenticate user, check service-tokens.admin permission
    → handleCreateToken:
