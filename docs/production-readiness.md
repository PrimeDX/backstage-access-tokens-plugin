...
- [ ] Your permission policy handles `service-tokens.admin` — either as a standalone policy or merged into an existing one (see [Merging into an existing permission policy](#merging-into-an-existing-permission-policy))
...
Add the `service-tokens.admin` check to your existing policy's `handle` method:
...
// AFTER — with service-tokens.admin merged in
class MyExistingPolicy implements PermissionPolicy {
...
2. If not, wrap the generated policy: create a thin `WrappedPolicy` that handles `service-tokens.admin` itself and delegates everything else to the generated policy.
...
- [ ] The `service-tokens.admin` permission is handled in your active permission policy
