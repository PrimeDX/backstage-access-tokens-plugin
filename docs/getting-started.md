...
The plugin enforces the `service-tokens.admin` permission on every token management endpoint. This requires the Backstage permission framework to be installed in your backend.
...
> **If you already have a permission policy**, you do not need a separate install — just add the `service-tokens.admin` check to your existing policy (see Step 4).
...
The plugin exposes a single permission: `service-tokens.admin`. Every API endpoint requires this permission. You must wire it into your Backstage permission policy.
...
> **If you already have a permission policy**, add the `service-tokens.admin` check to your existing `handle` method rather than creating a new policy. Backstage only supports one active permission policy at a time.
...
This grants the `service-tokens.admin` permission to the listed users. Users not in this list will receive `403 Forbidden` when accessing any service token endpoint.
