...
export function createKnexServiceTokenDatabase(options) {
  ...
  return {
    async findActiveToken(tokenHash) {
      const row = await client('service_tokens')
    ...
    async createTokenRecord(record) {
      await client('service_tokens').insert(toTokenRow(record));
    },

    async getTokenRecord(id) {
      const row = await client('service_tokens').where({ id }).first();
      return row ? mapTokenRow(row) : null;
    },
    ...
    async listTokens(filters = {}, options = {}) {
      ...
      const rows = await client('service_tokens').orderBy('created_at', 'desc');
      const tokens = rows
        .map(mapTokenRow)
    ...
    async appendAuditEvent(event) {
      await client('service_token_audit_log').insert({
        id: event.id,
    ...
    async getAuditLog(tokenId) {
      const rows = await client('service_token_audit_log')
