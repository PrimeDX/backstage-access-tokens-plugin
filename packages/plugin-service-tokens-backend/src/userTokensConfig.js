...
/**
 * Read and validate the `serviceTokens.userTokens.*` configuration block.
 *
...
export function readUserTokensConfig(rootConfig) {
  const root = rootConfig?.getOptionalConfig?.('serviceTokens.userTokens');
  if (!root) {
    throw new Error(
      'serviceTokens.userTokens config block is required to enable the user-tokens capability',
    );
  }
  ...
  if (
    ...
    throw new Error(
      `serviceTokens.userTokens.defaultExpiryDays must be a positive number ≤ maxExpiryDays (${maxExpiryDays})`,
    );
  }
  if (!Number.isFinite(maxExpiryDays) || maxExpiryDays <= 0) {
    throw new Error('serviceTokens.userTokens.maxExpiryDays must be a positive number');
  }
