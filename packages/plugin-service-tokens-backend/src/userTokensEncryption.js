...
export function decodeEncryptionKey(base64Key) {
  if (typeof base64Key !== 'string' || base64Key.length === 0) {
    throw new Error(
      'serviceTokens.userTokens.encryptionKey is required to use the user-tokens capability',
    );
  }
  ...
  try {
    ...
    throw new Error('serviceTokens.userTokens.encryptionKey must be valid base64');
  }

  if (raw.length !== KEY_BYTES) {
    throw new Error(
      `serviceTokens.userTokens.encryptionKey must decode to ${KEY_BYTES} bytes; got ${raw.length}`,
    );
  }
 ...
 *   suitable for direct insertion into the user_tokens table.
 */
...
/**
 * Decrypt the AES-256-GCM components retrieved from the user_tokens table
