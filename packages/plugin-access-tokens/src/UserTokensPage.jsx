...
export function UserTokensContent() {
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      const url = await discoveryApi.getBaseUrl('access-tokens');
  ...
  //   #personal-access-tokens-mint=<base64-payload> on success, or
  //   #personal-access-tokens-mint-error=<base64-detail> on failure.
  ...
  useEffect(() => {
    ...
    const errorPrefix = '#personal-access-tokens-mint-error=';
  ...
  const reload = useCallback(async () => {
    ...
    setLoadError(null);
    try {
      const res = await fetchApi.fetch(`${baseUrl}/personal`);
      if (!res.ok) {
  ...
  const onMintSubmit = useCallback(
    async input => {
      const res = await fetchApi.fetch(`${baseUrl}/personal/mint`, {
        method: 'POST',
  ...
  const onRevokeConfirm = useCallback(async () => {
    ...
    try {
      const res = await fetchApi.fetch(
        `${baseUrl}/personal/${encodeURIComponent(revokeTarget.id)}`,
        { method: 'DELETE' },
      );
  ...
  return h(
    ...
    h(
      ...
      h(
        ...
        'Personal access tokens are user-managed Backstage refresh tokens. Exchange them for short-lived Backstage API tokens that authenticate as your user principal.',
      ),
      h(
        Button,
        {
...
export function UserTokensPage() {
  return h(
    ...
    h(Header, {
      ...
      subtitle: 'User-managed Backstage refresh tokens',
    }),
    h(
      Content,
      null,
