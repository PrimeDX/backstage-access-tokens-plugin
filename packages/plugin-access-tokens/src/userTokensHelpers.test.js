...
test('parseMintResultFragment returns null for non-matching hashes', () => {
  ...
  assert.equal(parseMintResultFragment('personal-access-tokens-mint=xxx'), null);
});

test('parseMintResultFragment returns null for malformed base64', () => {
  assert.equal(parseMintResultFragment('#personal-access-tokens-mint=!!!'), null);
});
...
test('parseMintResultFragment returns null for JSON without required shape', () => {
  ...
  assert.equal(parseMintResultFragment(`#personal-access-tokens-mint=${badShape}`), null);
  ...
  assert.equal(parseMintResultFragment(`#personal-access-tokens-mint=${missingToken}`), null);
  ...
  assert.equal(parseMintResultFragment(`#personal-access-tokens-mint=${missingMetadata}`), null);
});
...
test('parseMintResultFragment accepts a well-formed payload', () => {
  ...
  const out = parseMintResultFragment(
    `#personal-access-tokens-mint=${encodeFragmentPayload(payload)}`,
  );
