...
export function ServiceTokensPage() {
  ...
  const loadTokens = useCallback(
    async (activeFilters = {}) => {
      ...
      setError(null);
      try {
        const baseUrl = await discoveryApi.getBaseUrl('access-tokens');
        const query = buildListQuery(activeFilters);
        const response = await fetchApi.fetch(`${baseUrl}/service${query}`);
        if (!response.ok) {
  ...
  useEffect(() => {
    ...
    async function loadMeta() {
      try {
        const baseUrl = await discoveryApi.getBaseUrl('access-tokens');
        const scopesRes = await fetchApi.fetch(`${baseUrl}/service/scopes`);
        if (scopesRes.ok) {
  ...
  async function handleCreateSubmit() {
    ...
    try {
      ...
      const response = await fetchApi.fetch(`${baseUrl}/service`, {
        method: 'POST',
  ...
  async function handleRevokeConfirm() {
    ...
    try {
      const baseUrl = await discoveryApi.getBaseUrl('access-tokens');
      const response = await fetchApi.fetch(
        `${baseUrl}/service/${encodeURIComponent(revokeToken.id)}`,
        {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function handleOpenAudit(token) {
    ...
    try {
      const baseUrl = await discoveryApi.getBaseUrl('access-tokens');
      const response = await fetchApi.fetch(
        `${baseUrl}/service/${encodeURIComponent(token.id)}/audit`,
      );
      if (!response.ok) {
