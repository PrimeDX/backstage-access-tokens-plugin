 ...
 *     this happens after the page reloads with a #personal-access-tokens-mint
...
export function CreateUserTokenDialog({
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // /settings/personal-tokens#personal-access-tokens-mint=<payload>, the page
    ...
    return h(
      ...
      h(
        ...
        h(
          ...
          'This Backstage refresh token will not be shown again. Copy it now and store it somewhere safe.',
        ),
        h(
          Alert,
          { severity: 'info', style: { marginBottom: 12 } },
          'Exchange this refresh token at /api/auth/v1/token for a short-lived Backstage API token, then use that API token as the bearer token. Do not send this refresh token directly as an API bearer token.',
        ),
        h(
          Box,
        ...
        h(
          ...
          `Backstage refresh token "${prefilledResult.metadata.name}" · expires ${prefilledResult.metadata.expiresAt}`,
        ),
        h(
          Typography,
          {
          ...
          'Any client that can make HTTP requests can use this flow. The resulting API token authenticates as your user principal.',
        ),
      ),
      h(
        DialogActions,
  ...
  return h(
    ...
    h(
      ...
      h(
        Alert,
        { severity: 'info', style: { marginBottom: 12 } },
        'A personal access token is a user-managed Backstage refresh token. Integrations exchange it for short-lived Backstage API tokens that authenticate as your user principal.',
      ),
      h(TextField, {
        fullWidth: true,
        autoFocus: true,
