 ...
 * serviceTokenHandlerModule factory has run). Returns a function that
...
export const serviceTokenHandlerModule = createServiceFactory({
  ...
  async factory({ config, lifecycle, logger }, databaseManager) {
    ...
    const database = databaseManager.forPlugin('service-tokens', { lifecycle, logger });
