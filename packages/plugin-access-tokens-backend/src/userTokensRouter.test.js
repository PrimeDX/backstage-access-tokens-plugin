...
function makeRouter(overrides = {}) {
  ...
  const router = createUserTokensRouter({
    ...
    encryptionKey,
    userTokensConfig,
    getExternalBaseUrl: async () => 'https://example/api/access-tokens',
    generateId,
    now: overrides.now,
  });

  const app = express();
  app.use('/api/access-tokens', router);
...
async function requestJson(app, method, path, options = {}) {
  ...
  req.method = method;
  req.url = `/api/access-tokens${path}`;
  req.headers = headers;
...
test('POST /personal/mint without session returns 401', async () => {
  const { app } = makeRouter();
  const r = await requestJson(app, 'POST', '/personal/mint', {
    body: { name: 'a' },
  });
  assert.equal(r.status, 401);
});

test('POST /personal/mint without write permission returns 403', async () => {
...
test('POST /personal/mint rejects missing name', async () => {
  const { app } = makeRouter();
  const r = await requestJson(app, 'POST', '/personal/mint', {
    headers: { 'x-user-ref': 'user:default/alice' },
    body: {},
...
test('POST /personal/mint rejects expiresAt in the past', async () => {
...
test('POST /personal/mint rejects duplicate name for the same user', async () => {
...
test('POST /personal/mint happy path returns flowId, state and authorizeUrl', async () => {
...
test('GET /personal/mint/callback redirects with error fragment on unknown state', async () => {
  const { app } = makeRouter();
  const r = await requestJson(app, 'GET', '/personal/mint/callback?code=c&state=unknown');
  assert.equal(r.status, 302);
  assert.match(
    r.headers?.location,
    /\/settings\/personal-tokens#personal-access-tokens-mint-error=/,
  );
  const encoded = r.headers.location.split('#personal-access-tokens-mint-error=')[1];
  const decoded = JSON.parse(Buffer.from(encoded, 'base64url').toString('utf8'));
  assert.match(decoded.message, /unknown or expired/);
});

test('GET /personal/mint/callback redirects with error fragment when OAuth returned an error', async () => {
  const { app } = makeRouter();
  const r = await requestJson(
    app,
    'GET',
    '/personal/mint/callback?error=access_denied',
  );
  assert.equal(r.status, 302);
  assert.match(
    r.headers?.location,
...
test('GET /personal/mint/callback happy path inserts row + audit, returns HTML with token', async () => {
  ...
  const r = await requestJson(
    app,
    'GET',
    `/personal/mint/callback?code=AUTHCODE&state=${encodeURIComponent(created.state)}`,
  );
  ...
  assert.match(
    location,
    new RegExp(
      `^https://frontend\\.example\\.com/settings/personal-tokens#personal-access-tokens-mint=`,
    ),
  );
  const encoded = location.split('#personal-access-tokens-mint=')[1];
  const decoded = JSON.parse(Buffer.from(encoded, 'base64url').toString('utf8'));
  assert.equal(decoded.type, 'personal-access-tokens-mint-result');
...
test('GET /personal lists only the caller\'s tokens', async () => {
  const { app, db } = makeRouter();
  await db.createUserToken({
    id: 't1',
    name: 'mine',
  ...
  const r = await requestJson(app, 'GET', '/personal', {
    headers: { 'x-user-ref': 'user:default/alice' },
  });
  assert.equal(r.status, 200);
...
test('GET /personal/:id returns 404 for another user\'s id', async () => {
  const { app, db } = makeRouter();
  await db.createUserToken({
    id: 't1',
    name: 'alice-token',
  ...
  const r = await requestJson(app, 'GET', '/personal/t1', {
    headers: { 'x-user-ref': 'user:default/bob' },
  });
  assert.equal(r.status, 404);
});

test('DELETE /personal/:id revokes via oauth and wipes ciphertext', async () => {
  ...
  const r = await requestJson(app, 'DELETE', '/personal/t1', {
    headers: { 'x-user-ref': 'user:default/alice' },
  });
...
test('DELETE /personal/:id is idempotent on already-revoked tokens', async () => {
  const db = createInMemoryUserTokensDatabase();
  await db.createUserToken({
    id: 't1',
    name: 'gone',
...
test('DELETE /personal/:id returns 502 when oauth revoke rejects', async () => {
  const encryptionKey = randomBytes(32);
...
test('GET /personal/:id/audit returns events newest-first', async () => {
  const db = createInMemoryUserTokensDatabase();
  await db.createUserToken({
    id: 't1',
    name: 'a',
  ...
  const { app } = makeRouter({ db });
  const r = await requestJson(app, 'GET', '/personal/t1/audit', {
    headers: { 'x-user-ref': 'user:default/alice' },
  });
  assert.equal(r.status, 200);
  assert.deepEqual(
