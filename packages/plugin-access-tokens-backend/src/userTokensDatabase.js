/**
 * Persistence layer for the personal-access-token capability. Mirrors the structure
 * of `database.js` (access-tokens) but with a separate set of tables and
...
export function createKnexUserTokensDatabase(options) {
  ...
  return {
    async createUserToken(record) {
      await client('personal_access_tokens').insert(toUserTokenRow(record));
    },

    async findUserTokenByUserAndName(userEntityRef, name) {
      const row = await client('personal_access_tokens')
    ...
    async listUserTokensForUser(userEntityRef, listOptions = {}) {
      const currentTime = listOptions.now ?? now();
      const rows = await client('personal_access_tokens')
    ...
    async updateUserTokenLastUsed(id, when) {
      ...
      await client('personal_access_tokens').where({ id }).update({ last_used_at: value });
    },

    async markUserTokenRevoked(id, when) {
      const updated = await client('personal_access_tokens').where({ id }).update({
    ...
    async appendUserAuditEvent(event) {
      await client('personal_access_token_audit_log').insert({
        id: event.id,
    ...
    async getUserAuditLog(tokenId) {
      const rows = await client('personal_access_token_audit_log')
    ...
    async getDcrClient() {
      const row = await client('personal_access_tokens_dcr_client').first();
      if (!row) return null;
      return {
        id: row.id,
    ...
    async saveDcrClient(record) {
      ...
      await client.transaction(async trx => {
        await trx('personal_access_tokens_dcr_client').del();
        await trx('personal_access_tokens_dcr_client').insert({
          id: record.id,
