...
/**
 * OAuth orchestration helper that lets the personal-access-token capability obtain
 ...
 *
 * @param {object} deps
 * @param {object} deps.db                 personal-access-token database (for DCR cache)
...
export function createOauthOrchestrator(deps) {
  ...
  async function ensureDcrClient(input) {
    ...
    logger?.info?.('Registered DCR client for personal access tokens', {
