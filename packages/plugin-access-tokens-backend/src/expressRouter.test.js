...
test('express router persists and serves token lifecycle through the real sqlite database', async () => {
  ...
  try {
    ...
    const app = createApp(
      ...
      assert.equal(created.body.rawToken, 'bsat_dGhpcyBpcyBub3QgYSByZWFsIHRva2Vu');
...
function createApp(router) {
  const app = express();
  app.use('/api/access-tokens/service', router);
  return app;
}
...
async function requestJson(app, method, path, options = {}) {
  ...
  req.method = method;
  req.url = `/api/access-tokens/service${path}`;
  req.headers = headers;
